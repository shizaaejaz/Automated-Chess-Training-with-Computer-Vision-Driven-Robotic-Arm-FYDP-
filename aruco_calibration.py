...
def compute_warp_matrix(marker_centers):
    ...
    W = WARP_SIZE
    src_pts = np.float32([
        marker_centers[12],  # TL
        marker_centers[13],  # TR
        marker_centers[11],  # BR
        marker_centers[10],  # BL
    ])
    dst_pts = np.float32([
...
# ── Cache I/O ─────────────────────────────────────────────────────────────────
def save_cache(marker_centers, warp_matrix, source_size=None):
...
def main():
    ...
    while not calibration_done:
        ...
        if key == ord('q'):
            ...
            h, w = frame.shape[:2]
            save_cache(locked_centers, M, source_size=[w, h])
