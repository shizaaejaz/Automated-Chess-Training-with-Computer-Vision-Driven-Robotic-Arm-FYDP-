...
ARM_WAIT_SEC  = 10     # robot arm ke settle hone ka wait
...
def robot_turn(brain, timer, r_str, last_confirmed_fen: str) -> str:
    """
    Robot ka poora turn:
      1. Stockfish → best move nikalo
      2. Robot arm ko bhejo (uncomment when ready)
      3. ARM_WAIT_SEC wait karo
      4. new_fen return karo

    Returns: new_fen (robot ke move ke baad ki position)
             ya last_confirmed_fen agar kuch fail ho
    """
    sep()
    print("  🤖 ROBOT KA TURN")
    sep()
    ...
    # Step R1: Stockfish
    step("R1", f"Stockfish calculate kar raha hai (difficulty: {DIFFICULTY})...")
    info(f"FEN bhej raha hoon: {last_confirmed_fen}")

    brain_result = brain.process_turn(last_confirmed_fen, difficulty=DIFFICULTY)

    if not brain_result["success"]:
        fail(f"Stockfish fail: {brain_result['error']}")
        return last_confirmed_fen   # same FEN rakhte hain, retry hoga

    ok(f"Robot move   : {brain_result['robot_move']}")
    info(f"Human hints  : {brain_result['human_hints']}")
    info(f"New FEN      : {brain_result['new_fen']}")

    # Redis update
    r_str.set("robot_move",  brain_result["robot_move"])
    r_str.set("human_hints", str(brain_result["human_hints"]))
    r_str.set("board_fen",   brain_result["new_fen"])
    ok("Redis mein robot_move + new FEN save ho gaya")

    # Step R2: Robot arm
    step("R2", "Robot arm ko move bhej raha hoon...")
    # ── Uncomment when robot_coordinator ready: ──
    # from robot_coordinator import execute_move
    # execute_move(brain_result["robot_move"])
    warn(f"[SIMULATION] Arm move: {brain_result['robot_move']} (abhi commented out)")

    # Step R3: Wait — arm settle ho jaye
    step("R3", f"Arm ke settle hone ka wait ({ARM_WAIT_SEC} sec)...")
    for i in range(ARM_WAIT_SEC, 0, -1):
        print(f"\r         ⏳ {i} sec...", end="", flush=True)
        time.sleep(1)
    print()
    ok("Arm move complete — Human ki baari shuru")

    # Timer: White ruko, Black chalu
    timer.on_white_move_done()
    ok("⚪ White timer RUKA")
    ok("⚫ Black timer CHALU — Human ki baari")

    return brain_result["new_fen"]
...
def run():
    ...
    last_confirmed_fen = robot_turn(brain, timer, r_str, last_confirmed_fen)
    ...
    while True:
        try:
            ...
            # ══════════════════════════════════════════════
            step(1, "Redis se latest frame fetch kar raha hoon...")
            r_str.set("pipeline_status", "step1_fetch")

            frame_bytes, frame_id = get_latest_frame(r_bin, r_str)

            if frame_bytes is None:
                warn("Redis mein koi frame nahi — camera_capture_android.py chala hai?")
                continue

            info(f"Frame mila → frame_id: {frame_id} | size: {len(frame_bytes)//1024} KB")

            # ══════════════════════════════════════════════
            # STEP 2 — Same frame? Skip
            # ══════════════════════════════════════════════
            step(2, "Nayi frame hai ya same wali?")

            if frame_id == last_frame_id:
                info(f"Same frame_id ({frame_id}) — camera ne naya frame nahi diya abhi")
                continue

            last_frame_id = frame_id
            ok(f"Naya frame! frame_id: {frame_id}")

            # ══════════════════════════════════════════════
            # STEP 3 — YOLO  [warp_matrix ab pass ho raha hai]
            # ══════════════════════════════════════════════
            step(3, "YOLO chala raha hoon — pieces detect + FEN bana raha hoon...")
            r_str.set("pipeline_status", "step3_yolo")

            # ── CHANGE: warp_matrix pass kar rahe hain ────
            result = process_frame(
                frame_bytes,
                warp_matrix=warp_matrix,   # ← ArUco se aaya perspective fix
                visualise=False
            )

            if not result["success"]:
                fail(f"YOLO fail: {result['error']}")
                continue

            new_fen = result["fen"]
            ok(f"YOLO FEN  : {new_fen}")
            info(f"Pieces    : {result['pieces']}")
            ...
            # ══════════════════════════════════════════════
            step(5, "CHECK 2 — Board badla? last_confirmed_fen se compare...")
            r_str.set("pipeline_status", "step5_check2")

            new_pos  = new_fen.split(" ")[0]
            prev_pos = last_confirmed_fen.split(" ")[0]

            info(f"Prev pieces : {prev_pos[:40]}...")
            info(f"New  pieces : {new_pos[:40]}...")

            if new_pos == prev_pos:
                info("FEN same hai — koi move nahi hua (board same)")
                candidate_fen = ""   # reset
                var("candidate_fen", repr(candidate_fen))
                continue
            ...
            sep()
            print("  🧑 HUMAN MOVE ACCEPTED (NO VALIDATION MODE)")
            sep()
            ...
            last_confirmed_fen = robot_turn(
                brain, timer, r_str, last_confirmed_fen
            )
            ...
            sep()
            print("  🔄 HUMAN KI BAARI — YOLO dobara chalu")
            sep()
